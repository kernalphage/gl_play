...
void drawNoise(Processing* ctx, bool& redraw, bool& clear, int curFrame, int maxFrames){
  STATIC_DO_ONCE(clear = true);
  static FastNoise n;
  static RandomCache<float> rx(1000, 2, [=](vec2 pos){return n.GetValueFractal(pos.x, pos.y, 0);});
  static RandomCache<float> ry(1000, 2, [=](vec2 pos){return n.GetValueFractal(pos.x, pos.y, 100);});
