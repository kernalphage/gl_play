...
public:
    ...
    void load(const char* filename){
      ...
      if(pixels == nullptr){
        std::cout<<"Pixels failed to load "<< filename<<std::endl;
      }
         if(uploadToGPU){
    ...
    unsigned char sample(float u, float v, int channel){
        int x = glm::round(texWidth * u);
        int y = glm::round(texHeight * v);
        x = glm::clamp(x, 0, texWidth-1);
        y = glm::clamp(y, 0, texHeight-1);
        float ret =  pixels[(y * texWidth + x) * texChannels + channel];
      return ret;
    }
